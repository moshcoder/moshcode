...
async function answerSession(auth, args = {}) {
  ...
  return sendSession(auth, { session_id: args.session_id, text });
}

async function approveSession(auth, args = {}) {
  if (!["approve", "deny"].includes(args.decision)) throw new Error("decision must be approve or deny");
  return sendSession(auth, { session_id: args.session_id, text: args.decision === "approve" ? "yes" : "no" });
}

async function cancelSession(auth, args = {}) {
  const row = await sessionFor(auth, args.session_id);
  if (!isLive(row)) throw new Error("session is offline");
...
const TOOL_DEFS = [
  ...
  },
  {
    ...
    title: "Answer a Moshcode prompt",
    description: "Answer the question currently waiting in a live Moshcode session with one bounded line.",
    inputSchema: {
      type: "object",
      properties: {
        session_id: { type: "string", description: "Live Moshcode session id." },
  ...
  },
  {
    ...
    title: "Approve a Moshcode confirmation",
    description: "Approve or deny the confirmation currently waiting in a live Moshcode session.",
    inputSchema: {
      type: "object",
      properties: {
        session_id: { type: "string", description: "Live Moshcode session id." },
  ...
  },
  {
    ...
    title: "Interrupt a Moshcode session",
    description: "Send Ctrl-C to the process currently holding the live Moshcode terminal.",
    inputSchema: {
      type: "object",
      properties: { session_id: { type: "string", description: "Live Moshcode session id." } },
      required: ["session_id"],
      additionalProperties: false,
    },
...
async function invokeTool(auth, name, args) {
  ...
  if (name === "moshcode_session_answer") return answerSession(auth, args);
  if (name === "moshcode_session_approve") return approveSession(auth, args);
  if (name === "moshcode_session_cancel") return cancelSession(auth, args);
...
async function handleMcp(req, res) {
  ...
  try {
    ...
    if (error.requiredScope) {
      scopeChallenge(res, error.requiredScope, req.mcpAuth?.resource);
      res.status(403);
    }
...
mcpRouter.post("/mcp", requireMcpAccess, handleMcp);
...
mcpRouter.post("/api/v1/mcp/shares", apiUser, async (req, res) => {
  const session = await get(
    `SELECT * FROM cli_sessions WHERE id=? AND user_id=?`,
    [String(req.body?.session_id || ""), req.apiUser.id]
  );
  ...
  res.status(201).json({
    ...
    endpoint: mcpShareResource(shareId),
    scopes,
...
mcpRouter.get("/api/v1/mcp/shares", apiUser, async (req, res) => {
  const rows = await all(
    `SELECT sh.*, s.status AS session_status, s.last_seen_at
...
mcpRouter.post("/api/v1/mcp/:shareId", requireMcpShareAccess, handleMcp);
