...
mcpOAuthBrowserRouter.get("/oauth/authorize", requireAuth, async (req, res) => {
  try {
    ...
    const sessions = await sessionsForAuthorization(req.user.id);
    ...
    res.type("html").send(page({
      ...
      body: `<main class="oauth">
        <div class="oauth-card">
          ...
          <p>This client is asking to connect to <b>${esc(MCP_RESOURCE)}</b> as ${esc(req.user.email || req.user.display_name || "you")}.</p>
          <h3>Permissions</h3>
          <ul class="scope-list">${scopeRows}</ul>
          <h3>Session access</h3>
          <p class="dim">Bind this authorization to one session for least privilege, or allow all of your sessions.</p>
